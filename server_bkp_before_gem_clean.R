...
server <- function(input, output, session) {
    ...
    # ---- 🚀 SELECT SPECIES REACTIVE: filtered_data ---
    ...
    # ----
    #  OLD STUFF STILL TRYING TO UNDERSTAND WHAT IS USEFUL WHAT IS THRASH...
    ...
    output$right_panel_container <- renderUI({
        ...
        if (!is.null(top_lvl) && top_lvl == "SCAN Analysis") {
            ...
            # CASE C: SCAN TAB (Right Panel: Status & Results Only) ----
            ...
            has_results <- FALSE
            try({
                if(exists("scan_graph") && !is.null(scan_graph())) has_results <- TRUE
            }, silent=TRUE)
   ...
   # ---- CS MINI INSPECTOR (Right Panel) ---
    ...
    output$scan_chorotype_list <- renderTable({
      ...
      df <- data.frame(
        Species = names(comps$membership),
        Chrotype = as.integer(comps$membership)
      )
      
      # Sort by Group ID, then Species Name
      df <- df %>% arrange(Chorotype, Species)
